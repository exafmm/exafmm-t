...
namespace exafmm_t {
  ...
  void hadmard_kernel(int *d_M2Ltargets_idx, cufftComplex *d_up_equiv_fft, cufftComplex *d_dw_equiv_fft, int *d_M2LRelPos_offset, int *d_index_in_up_equiv_fft, int *d_M2LRelPoss, real_t *d_mat_M2L_Helper, int n3_, int BLOCKS, int *d_M2Llist_idx_offset, int *d_M2Llist_idx) {
    int i = blockIdx.x;
    int k = threadIdx.x;
    extern __shared__ cufftComplex check[];
    check[k].x = 0;
    check[k].y = 0;
    __syncthreads();
    ...
    for(int j=0; j <M2Llist_size; j++) {
      ...
      d_dw_equiv_fft[i*n3_+k].x += check[k].x;
      d_dw_equiv_fft[i*n3_+k].y += check[k].y;
  }
  ...
  cufftComplex *HadmardGPU(std::vector<int> &M2Ltargets_idx, std::vector<int> &M2LRelPos_offset, std::vector<int> &index_in_up_equiv_fft, std::vector<int> &M2LRelPoss, RealVec mat_M2L_Helper, int n3_, cufftComplex *d_up_equiv_fft, std::vector<int> &M2Llist_idx_offset, std::vector<int> &M2Llist_idx, int max) {
    int BLOCKS = M2Ltargets_idx.size();
    int THREADS = n3_;

    int *d_M2Ltargets_idx, *d_M2LRelPos_offset, *d_index_in_up_equiv_fft, *d_M2LRelPoss, *d_M2Llist_idx_offset, *d_M2Llist_idx;
    ...
    hadmard_kernel<<<BLOCKS, THREADS, sizeof(cufftComplex)*n3_>>>(d_M2Ltargets_idx, d_up_equiv_fft, d_dw_equiv_fft, d_M2LRelPos_offset, d_index_in_up_equiv_fft, d_M2LRelPoss, d_mat_M2L_Helper, n3_, BLOCKS, d_M2Llist_idx_offset, d_M2Llist_idx);
    gpuErrchk( cudaPeekAtLastError() );
    gpuErrchk( cudaDeviceSynchronize() );
    Profile::Toc();
  ...
  std::vector<real_t> M2LGPU(std::vector<int> &M2Ltargets_idx, std::vector<int> &M2LRelPos_offset, std::vector<int> &index_in_up_equiv_fft, std::vector<int> &M2LRelPoss, RealVec mat_M2L_Helper, std::vector<int> &M2Lsources_idx, std::vector<real_t> &upward_equiv, std::vector<int> &M2Llist_idx_offset, std::vector<int> &M2Llist_idx, int max) {
    ...
    cufftComplex *d_dw_equiv_fft = HadmardGPU(M2Ltargets_idx, M2LRelPos_offset, index_in_up_equiv_fft, M2LRelPoss, mat_M2L_Helper, n3_, d_up_equiv_fft, M2Llist_idx_offset, M2Llist_idx, max);
  ...
  /*void L2PGPU(std::vector<real_t> &dnwd_equiv_surf, RealVec &dnward_equiv, std::vector<real_t> &bodies_coord, std::vector<real_t> &nodes_trg, std::vector<int> &leafs_idx, std::vector<int> &nodes_pt_src_idx) {
    ...
    real_t *d_dnward_equiv, *d_L2L_U, *d_buffer, *d_L2L_V, *d_bodies_coord, *d_nodes_trg, *d_dnwd_equiv_surf;
    int *d_leafs_idx, *d_nodes_pt_src_idx;
    ...
    cudaMalloc(&d_leafs_idx, sizeof(int)*leafs_idx.size());
    cudaMalloc(&d_nodes_pt_src_idx, sizeof(int)*nodes_pt_src_idx.size());
    cudaMalloc(&d_dnwd_equiv_surf, sizeof(real_t)*dnwd_equiv_surf.size());
    
    cudaMemcpy(d_dnwd_equiv_surf, &dnwd_equiv_surf[0], sizeof(real_t)*dnwd_equiv_surf.size(), cudaMemcpyHostToDevice);
    cudaMemcpy(d_dnward_equiv, &dnward_equiv[0], sizeof(real_t)*dnward_equiv.size(), cudaMemcpyHostToDevice);
    cudaMemcpy(d_L2L_U, &L2L_U[0], sizeof(real_t)*L2L_U.size(), cudaMemcpyHostToDevice);
    cudaMemcpy(d_L2L_V, &L2L_V[0], sizeof(real_t)*L2L_V.size(), cudaMemcpyHostToDevice);
    cudaMemcpy(d_bodies_coord, &bodies_coord[0], sizeof(real_t)*bodies_coord.size(), cudaMemcpyHostToDevice);
  ...
  void fmmStepsGPU(Nodes& nodes, std::vector<int> &leafs_idx, std::vector<real_t> &bodies_coord, std::vector<real_t> &nodes_pt_src, std::vector<int> &nodes_pt_src_idx, int ncrit, RealVec &upward_equiv, std::vector<std::vector<int>> &nodes_by_level_idx, std::vector<std::vector<int>> &parent_by_level_idx, std::vector<std::vector<int>> &octant_by_level_idx, std::vector<real_t> &nodes_coord, std::vector<int> &M2Lsources_idx, std::vector<int> &M2Ltargets_idx, RealVec &dnward_equiv, std::vector<real_t> &nodes_trg, std::vector<int> &nodes_depth, std::vector<int> &nodes_idx) {
    ...
    cublasHandle_t handle;
    cublasCreate(&handle);
    int *d_nodes_pt_src_idx, *d_leafs_idx, *d_nodes_depth, *d_M2Lsources_idx, *d_M2Ltargets_idx, *d_nodes_idx;
