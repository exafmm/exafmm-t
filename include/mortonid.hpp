...
class MortonId{
 ...
 public:
  ...
  void NbrList(std::vector<MortonId>& nbrs,uint8_t level, int periodic) const{
	...
	if(newX<0) newX+=maxCoord; if(newX>=(int64_t)maxCoord) newX-=maxCoord;
	if(newY<0) newY+=maxCoord; if(newY>=(int64_t)maxCoord) newY-=maxCoord;
	if(newZ<0) newZ+=maxCoord; if(newZ>=(int64_t)maxCoord) newZ-=maxCoord;
	mid_tmp.x=newX; mid_tmp.y=newY; mid_tmp.z=newZ;
	mid_tmp.depth=level;
	nbrs.push_back(mid_tmp);
      }
