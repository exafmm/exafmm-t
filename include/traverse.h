...
namespace exafmm_t {
  void upwardPass(Nodes& nodes, std::vector<int> &leafs_idx, std::vector<real_t> &bodies_coord, std::vector<real_t> &nodes_pt_src, std::vector<int> &nodes_pt_src_idx, int ncrit, RealVec &upward_equiv, std::vector<std::vector<int>> &nodes_by_level_idx, std::vector<std::vector<int>> &parent_by_level_idx, std::vector<std::vector<int>> &octant_by_level_idx, std::vector<real_t> &nodes_coord, std::vector<int> &nodes_depth) {
    Profile::Tic("P2M", false, 5);
    P2M(nodes, leafs_idx, bodies_coord, nodes_pt_src, nodes_pt_src_idx, ncrit, upward_equiv, nodes_coord, nodes_depth);
    Profile::Toc();
    Profile::Tic("M2M", false, 5);
    M2M(nodes, upward_equiv, nodes_by_level_idx, parent_by_level_idx, octant_by_level_idx);
    Profile::Toc();
  }
