...
namespace exafmm_t {
  ...
  void buildTree(Body * bodies, Body * buffer, int begin, int end, Node * node, Nodes & nodes, const vec3 & X, real_t R, std::vector<int> &leafs_idx, std::vector<int> &nonleafs_idx, Args & args, int level=0, bool direction=false) {
    ...
    for (int i=0, c=0; i<8; i++) {
      ...
      if (size[i]) {
        ...
        buildTree(buffer, bodies, offsets[i], offsets[i] + size[i], &child[c++], nodes, Xchild, Rchild, leafs_idx, nonleafs_idx, args, level+1, !direction);
      }
    }
  }

  Nodes buildTree(Bodies & bodies, std::vector<int> &leafs_idx, std::vector<int> &nonleafs_idx, Args & args) {
    real_t R0 = 0.5;
    vec3 X0(0.5);
    ...
    buildTree(&bodies[0], &buffer[0], 0, bodies.size(), &nodes[0], nodes, X0, R0, leafs_idx, nonleafs_idx, args);
    return nodes;
  }
}
#endif
